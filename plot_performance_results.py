...
for i in range(2):
    ...
    destination_path = ""  # ENTER DESTINATION PATH

    if TYPE == "EXECTIME":
        average_performance = pickle.load(open( models[i] + "\\performance per particle number.pkl", 'rb'))
        ...
        average_error = pickle.load(open(+ models[i] + "\\errors per particle number.pkl",'rb'))
...
plt.legend()
plt.savefig("performance_"+TYPE+".svg")
