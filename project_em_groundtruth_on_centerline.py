...
CENTERLINE_ILIACA_UPPER_TOP = np.array([96.3498, 126.758, -336.572])
...
def project_single_point_on_centerline(point: np.ndarray):
    if distance_of_point_from_line(vessel=ILIACA, point=point) <= distance_of_point_from_line(vessel=AORTA,
                                                                                              point=point) \
            and distance_of_point_from_line(vessel=AORTA, point=point) <= distance_of_point_from_line(vessel=SIDE_BRANCH, point=point):
       ...
       # else:
        ...
        DISPLACEMENTS_FROM_ORIGIN.append(
            distance_3D(closest_point, SIDE_BRANCH_BASIS) + distance_3D(CENTERLINE_AORTA_TOP, SIDE_BRANCH_BASIS) + distance_3D(ILIACA_TOP, ILIACA_BASIS))
        PROJECTION_MERKER.append(closest_point.tolist())
    else:
